...
key = st.sidebar.selectbox("Choose Brand Column",
                     options=dataf.columns)



a = st.sidebar.selectbox("Choose Product Type",
...
chosen_level = st.sidebar.selectbox(
    "Choose " + str(b) + " Levels",
    options=dataf[b].unique()
)
  # SUBMISSION OF CHOICE:
df = dataf[dataf[key] == a]

df = df[[key, b]]
df = df[df[b] == chosen_level]
